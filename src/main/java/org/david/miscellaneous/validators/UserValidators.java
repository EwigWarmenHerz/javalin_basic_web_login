...
import org.david.miscellaneous.custom_exceptions.CustomExceptions.InvalidBodyException;
...
public class UserValidators {
  ...
  public static void userDtoValidator(Context ctx) {
    ...
    ctx.bodyValidator(UserModels.User.class)
        .check(
            user -> user.email() != null && !user.email().isBlank(), "Email can't be null or blank")
        .check(
            user -> user.password() != null && !user.password().isBlank(),
